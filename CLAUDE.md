...
- **`index.html`** — arcade mini-games (home page, `/`): Match It, Pop Balloons, Feed Bunny, Catch Stars, **Dino Eggs**, **Dino Dig**, **Draw**, **Princess**, **Candy House**, **Tardigrades**, **Red Riding Hood**, **Pull the Radish** (拔萝卜).
- **`learn.html`** — learning games: Find It, A is for…, **Books**, Trace It, **My Name**, Count It, Memory, Dot to Dot, Sort It, Shapes, **Count Dinos**. Dot to Dot also includes a **dinosaur** shape. Books includes **The Three Little Tardigrades** (original-words retelling of Avalynn's favourite book) and **Hansel and Gretel** (gentle kind-witch retelling of the public-domain Grimm tale).
...
- **Letter sounds / phonics (A is for… + Trace It):** prompts and praise say the letter's sound — `'B says bh! Which picture starts with B?'` / `'B says bh! B is for banana!'` (the latter clip is shared by Trace It's letter-finish). The phoneme spellings live in `const SOUND` (learn.html) and `SOUND` (generate_voice.py) — they **must stay identical**, and they were **human-auditioned against the Jenny voice** (Jon, 2026-07) because TTS pronounces phoneme spellings unpredictably. Don't change a spelling without re-auditioning; changing `VOICE` means re-auditioning all of them.
...
- **Tardigrades (index.html, added 2026-07):** guided story game from *The Three Little Tardigrades* (Sandra Fay) — `TARDI_HOMES` drives 3 scenes (Gavin/volcano, Colin/ice cave, Doug/moon; art `img/tardi-*.svg`, `spider-wolf.svg`, plus `spot-volcano/icecave.svg` + `tardi-trio.svg` used by the book). The wolf spider creeps toward the tardigrade (interval; it stops and waits — no fail state at age 3); tapping the tardigrade swaps it to its `-ball.svg` tun and the scene's emoji weather (`tardiFx`) chases the spider off (+1 ⭐). After all 3 homes: friendship finale — sad spider + 💧, tap him → hearts, +3 ⭐, all three tardigrades pop in, story loops. Pacing is `setTimeout` (index.html `say()` has no onDone callback); every async step guards `mode!=='tardi'`. ⚠️ The story text here and in the Books entry is an **original-words retelling** — never copy the actual book's text onto this public site.
- **Candy House (index.html, added 2026-07):** gentle Hansel & Gretel story game, three scenes that loop — ① candy trail: 6 🍬 with `.cbadge` numbers tapped in order (counting practice; `.pulseC` marks the next one, wrong order just wiggles); ② nibble the candy house: tap 6 candy emoji off `candy-house.svg` (yum clips); ③ kind witch finale: `candy-witch.svg` asks "Who is nibbling my house?", banner + tap her to share → +3 ⭐, `hansel-gretel.svg` pops in. All async steps guard `mode!=='candy'`. Art is hand-authored PlayBox SVG (`hansel-gretel.svg`, `candy-house.svg`, `candy-witch.svg`) — the witch is a **kind granny** (glasses, bun, cookie), per the "never scary" spec; the tale is public domain so no copyright concern, but keep the retelling gentle (no oven/eating-children).
- **Red Riding Hood (index.html, added 2026-09; drag+maze rework 2026-09):** gentle Grimm retelling, three scenes that loop — ① **pack the basket (drag & drop):** a kitchen (counter + shelf) scatters 6 items; a `.packList` names 3 to pack; **drag** the right ones into `basket.svg` (shared `startDrag`; a tap also works). Correct → speaks the item's English name (`RED_ITEMS`, 15 items each with a clip) + ✓; wrong → `say('Not that one')` + snap back. ② **forest maze:** a gentle grid maze (`edges`/`adj`, hand-authored, one winding path + two dead-end stubs); Red stops at every cell and the player taps a `.mazeArrow` (all open neighbours, **including backwards**) until she reaches the house 🏠 — wrong turns just walk into a stub, no penalty. ③ **kind wolf finale:** `wolf-kind.svg` looks hungry, banner + tap him to share bread → +3 ⭐, `grandma.svg` pops in. All async steps guard `mode!=='red'`. Art is hand-authored PlayBox SVG (`red-riding-hood`, `wolf-kind`, `grandma`, `basket`) — the wolf is **friendly, never scary**; no eating-anyone. Name-bearing intro needs a clip per profile.
- **Pull the Radish / 拔萝卜 (index.html, added 2026-09):** Chinese-voiced arithmetic story game. A giant `radish.svg` needs exactly the answer to a small +/− problem in helpers to pull out. Equation shown as numerals **plus countable dot-groups** (scaffold), spoken in Chinese. **Drag** helper chips onto the field to add them (shared `startDrag`; a tap works too), **tap a placed helper to send it back**. `RADISH_HELPERS` is 14 `{file,name}` (fun mascot names shown as labels, never spoken): Grandma, Hop, Rex, Pip/Pop/Pudge (3 distinct pig SVGs), Red, Billy (goat), Lily (girl), Sparkle (unicorn), Wolfie (`wolf-big.svg`), Trixie (`dino-trixie.svg`), Finn (`shark.svg`), Snowy (`polar-bear.svg`). Running count spoken 一…十. **拔萝卜! Pull** button evaluates: correct → radish pops out (`.radishPulled`), +3 ⭐, loops with a bigger number; too few → wiggle + "还不够", and after 2 tries `showRadishHint()` reveals the answer numeral (green pulse); too many → **giggly tumble** (`.helperTumble`) + retry, **no penalty, never injured** (deliberate — see design decision). Adaptive: answer range grows 3→6 with `radishStreak`; subtraction unlocks at ≥4. `R_ADD`/`R_SUB` pools **must stay identical to generate_voice.py**. ⚠️ Chinese clips are voiced by **`ZH_VOICE=zh-CN-XiaoxiaoNeural`** and keyed explicitly (Chinese text can't be slugified): JS plays them via **`sayKey('<key>')`** (not `say()`), and `generate_voice.py` maps `zh_*` keys → Chinese text with the Chinese voice. Adding a Chinese line = add the key in both places, regenerate.
