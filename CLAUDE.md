...
- **Red Riding Hood (index.html, added 2026-09):** gentle Grimm retelling, three scenes that loop — ① pack the basket: tap 5 goodies (🍞🧀🍎🍇🍪) in number order into `basket.svg` (`.cbadge`/`.pulseC` counting, wrong order wiggles); ② forest path: tap 5 flowers to walk `red-riding-hood.svg` rightward to Grandma's house 🏠; ③ kind wolf finale: `wolf-kind.svg` looks hungry, banner + tap him to share bread → +3 ⭐, `grandma.svg` pops in. All async steps guard `mode!=='red'`. Art is hand-authored PlayBox SVG (`red-riding-hood`, `wolf-kind`, `grandma`, `basket`) — the wolf is **friendly, never scary** (rosy cheeks, soft smile); no eating-anyone. Name-bearing intro needs a clip per profile.
- **Pull the Radish / 拔萝卜 (index.html, added 2026-09):** Chinese-voiced arithmetic story game. A giant `radish.svg` needs exactly the answer to a small +/− problem in helpers to pull out. Equation shown as numerals **plus countable dot-groups** (scaffold), spoken in Chinese. Tap helper chips to add/remove them from the rope (`RADISH_HELPERS` = grandma, bunny, dino, pig×3, red, goat, girl, unicorn; reuses existing art + new `radish/goat/girl/unicorn.svg`); running count spoken 一…十. **拔萝卜! Pull** button evaluates: correct → radish pops out (`.radishPulled`), +3 ⭐, loops with a bigger number; too few → wiggle + "还不够", and after 2 tries `showRadishHint()` reveals the answer numeral (green pulse); too many → **giggly tumble** (`.helperTumble`) + retry, **no penalty, never injured** (deliberate — see design decision). Adaptive: answer range grows 3→6 with `radishStreak`; subtraction unlocks at ≥4. `R_ADD`/`R_SUB` pools **must stay identical to generate_voice.py**. ⚠️ Chinese clips are voiced by **`ZH_VOICE=zh-CN-XiaoxiaoNeural`** and keyed explicitly (Chinese text can't be slugified): JS plays them via **`sayKey('<key>')`** (not `say()`), and `generate_voice.py` maps `zh_*` keys → Chinese text with the Chinese voice. Adding a Chinese line = add the key in both places, regenerate.
